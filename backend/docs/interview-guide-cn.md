...
- 增加本地稀疏向量余弦重排：通过 `EmbeddingClient` 把问题和 chunk 文本转成词袋 + 中文 bigram 的归一化稀疏向量，用余弦相似度作为额外排序信号。它不是神经网络 embedding，而是后续替换真实 embedding provider 的本地占位实现。
...
```text
我没有一开始就把项目绑定到某个向量数据库，而是先抽象 RetrievalStrategy 和 EmbeddingClient。当前主策略是 hybrid retrieval：先用 MySQL FULLTEXT、多关键词和元数据召回做可解释基线，再用本地稀疏向量余弦相似度做重排。这个本地向量不是神经网络 embedding，主要用于低成本验证混合检索流程；后续替换成真实 embedding API 或向量库时，也能用同一套评估集对比效果。
```
