...
- 增加本地 embedding-style 相似度重排：把问题和 chunk 文本转成归一化稀疏向量，用余弦相似度作为额外排序信号。
...
- 先把检索策略抽象好，再把本地 embedding 实现替换成真实 embedding provider 和向量库，风险更低。

可以这样回答：

```text
我没有一开始就把项目绑定到某个向量数据库，而是先抽象 RetrievalStrategy。当前主策略是 hybrid retrieval：先用 MySQL FULLTEXT、多关键词和元数据召回做可解释基线，再用本地 embedding-style 相似度做重排。这样可以先用 gold label 的 Hit@3/MRR 跑出基线，后续替换成真实 embedding 或向量库时，也能用同一套评估集对比效果。
```
