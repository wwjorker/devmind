...
另外，当前 RAG evaluation dataset 是代码中的标准 case 集合，不是数据库表。它用标准问题、期望关键词和期望答案检查系统是否召回正确上下文，不只是看模型有没有生成文字。检索评估接口会进一步输出 Hit@3、MRR 和首个相关片段排名，后续接入 embedding 或 rerank 时可以用同一批 case 做前后对比。
