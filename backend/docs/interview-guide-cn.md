...
```text
我没有一开始就把项目绑定到某个向量数据库，而是先抽象 RetrievalStrategy 和 EmbeddingClient。当前主策略是 hybrid retrieval：先用 MySQL FULLTEXT、多关键词和元数据召回做可解释基线，再用已持久化的本地稀疏向量做余弦重排。这个本地向量不是神经网络 embedding，主要用于低成本验证混合检索和向量索引流程；后续替换成真实 embedding API 或向量库时，也能用同一套评估集对比效果。
```
...
第一版先用关键词检索跑通主链路，便于调试和解释。现在已经做了多关键词、元数据召回、重复 chunk 降权、MySQL FULLTEXT，以及本地稀疏向量持久化后的混合检索雏形。它还不是外部 embedding + 向量数据库，下一步会接真实 embedding provider、向量库或 RRF/rerank，并用同一套 Hit@3/MRR 评估集对比效果。
...
4. 增加 RRF 或 rerank。
5. 基于当前 Hit@3/MRR 基线，对比外部 embedding、向量库和 rerank 的提升。
