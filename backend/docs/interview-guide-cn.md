...
另外，当前 RAG evaluation dataset 是代码中的标准 case 集合，不是数据库表。它用标准问题、期望关键词和期望答案检查系统是否召回正确上下文，不只是看模型有没有生成文字。
...
当前版本不是简单的单关键词 LIKE，而是可解释的关键词检索 V1。
...
- 后续可以平滑升级为 hybrid retrieval：关键词 + 向量 + rerank。

可以这样回答：

```text
我没有一开始就上向量库，是因为第一版先做可解释的检索闭环。现在的检索会解析多个关键词，并结合内容、标题、标签和类型算分，同时对重复 chunk 降权。后续再接入 embedding 做混合检索和 rerank。
```
...
- 设计 bad case feedback、RAG evaluation dataset 与前端评估看板，支持检索质量分析和迭代优化。
