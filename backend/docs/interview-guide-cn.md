...
- `knowledge_document_chunk_vector`：chunk 本地稀疏向量表。
- `ai_ask_log`：AI 问答日志表。
...
- chunk 向量和 chunk 一起重建并持久化，提问时只计算问题向量，避免每次请求都重新计算所有 chunk 的本地向量。
- `ai_ask_log` 和 `ai_ask_feedback` 分开，因为一次问答日志是客观调用记录，反馈是主观质量评价。
- 使用 Flyway 管理数据库迁移，避免依赖手动复制 SQL。

另外，当前 RAG evaluation dataset 是代码中的标准 case 集合，不是数据库表。它用标准问题、期望答案和人工标注的相关文档检查系统是否召回正确上下文，不只是看模型有没有生成文字。检索评估接口会输出 Hit@3、MRR 和首个相关片段排名；期望关键词只作为诊断展示，不作为相关性裁判。我已经用同一批 gold-label case 跑了四方对比（keyword / sparse-hybrid / dense-hybrid / dense-hybrid-rerank），结果见下方。

评估集中还加入了“词法失配”问题（例如不直接问“缓存穿透”，而是问“不存在的 key 一直打到 MySQL 怎么办”）、同义改写和跨主题 hard-negative，用来观察不同检索策略在改写和干扰场景下的差异。

**检索四方评估结果（14 篇文档、40 个 gold-label 用例、K=3）**

| 策略 | Hit@3 | MRR | 特点 |
|---|---|---|---|
| keyword baseline | 0.886 | 0.814 | 关键词/FULLTEXT，漏改写问题 |
| sparse-hybrid | 0.971 | 0.887 | 词法召回好 |
| dense-hybrid | 0.943 | 0.913 | 排序精度最强（最多命中第 1） |
| dense-hybrid-rerank | 1.000 | 0.867 | 最坏情况召回最强（答案永进前 3），但排序变平 |

最有意思的结论是无单一最优：dense embedding 排序最精确（MRR 最高），叠加 rerank 后 Hit@3 拉到满分——比如“不存在的 key 一直打到 MySQL”这个改写问题，关键词和向量都把正确的 Redis 文档排在第 5，rerank 提到了第 1；但 rerank 同时把好几个 dense 本来排在第 1 的简单 case 挤到了第 2，所以平均 MRR 反而降了。选哪个取决于产品更在乎“答案一定在前 3”还是“答案通常第 1”。

诚实的边界：评估集只有 40 条、用的是智谱的 reranker，是方向性结论、不是统计显著；rerank 只在离线评估里，没接进默认问答，避免每次线上请求都产生外部调用成本和延迟。

常见追问：
- 相关性怎么定义、会不会自己评自己？用人工标注的 gold 文档，不用检索器自己的关键词打分，避免循环论证。
- 为什么 rerank 反而降了 MRR？它救回了难 case（rank 5→1），但对 dense 本来精确排在第 1 的简单 case，偶尔把别的候选顶上来，把 gold 挤到第 2，拉低了平均排名。
- 为什么不直接把 rerank 接到线上问答？先离线量化收益和失败边界；rerank 每次要对候选逐一打分、有成本和延迟，确认值得再上线。
...
- 增加本地稀疏向量余弦重排：通过 `EmbeddingClient` 把问题和 chunk 文本转成词袋 + 中文 bigram 的归一化稀疏向量，用余弦相似度作为额外排序信号。chunk 向量在文档分块重建时写入 `knowledge_document_chunk_vector`，提问时只计算 query 向量并和已持久化的 chunk 向量比较。
- 用 RRF 融合关键词 / FULLTEXT 排名和本地稀疏向量排名，避免把不同量纲的检索分数直接相加。
- 用 `EmbeddingTextBuilder` 统一构造向量化输入文本，固定使用标题、来源类型、标签和 chunk 内容，避免后续接真实 embedding 或向量库时索引字段不一致。
...
- 先把检索策略和向量表示 provider 抽象好，再把本地稀疏向量实现替换成真实 embedding API 和向量库，风险更低。

可以这样回答：

```text
我没有一开始就把项目绑定到某个向量数据库，而是先抽象 RetrievalStrategy、EmbeddingClient 和 RerankClient。当前主策略是 hybrid retrieval：先用 MySQL FULLTEXT、多关键词和元数据召回做可解释基线，再用已持久化的向量做余弦相似度排序，并通过 RRF 融合两路排名。向量分两套：本地稀疏向量（词袋+中文 bigram，零成本默认）和真实 dense embedding（OpenAI 兼容 API，配 key 后启用），按 provider_name 在同一张向量表共存。我已经用同一套 gold-label 评估集对 keyword / sparse / dense / rerank 四方做过对比（结果见上文第 4 节表格），验证了这套抽象确实支撑"换检索策略、同集对比"的目标。下一步是把向量存储从 MySQL JSON 迁到真正的向量库。
```
...
第一版先用关键词检索跑通主链路，便于调试和解释。现在已经做了多关键词、元数据召回、重复 chunk 降权、MySQL FULLTEXT、本地稀疏向量持久化和 RRF 融合排序。它还不是外部 embedding + 向量数据库，下一步会接真实 embedding provider、向量库或专业 rerank，并用同一套 Hit@3/MRR 评估集对比效果。
...
1. 增加 BM25 或全文检索。
2. 接入外部 embedding provider 和向量数据库。
3. 将当前本地稀疏向量混合检索升级为外部 embedding provider 与 vector store。
4. 增加专业 rerank 或向量库召回。
5. 基于当前 keyword baseline 与 hybrid/RRF 的 Hit@3/MRR 基线，对比外部 embedding、向量库和 rerank 的提升。
