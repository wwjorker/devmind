...
当前版本不宣称已经实现外部 embedding、向量数据库、rerank、PDF/OCR 或生产级部署。第一版重点是先打通 Java 后端工程链路、RAG 问答闭环和质量评估流程；当前已加入本地稀疏向量持久化与混合检索雏形，为后续接真实向量检索做准备。
...
- Retrieval evaluation 检索评估，用标准问题和人工标注的相关文档验证关键词解析、Hit@3、MRR 和首个相关片段排名
...
4. 当检索不到有效上下文时，系统返回无上下文兜底，避免模型在知识库缺资料时编造答案。
5. Prompt Preview 与 Ask Log 会记录上下文、模型来源、召回片段、token 用量、耗时和状态，方便定位 RAG 问题。
6. `LlmClient` 抽象隔离业务流程和模型供应商，支持 Mock、本地测试、DeepSeek 接入和后续 Provider 扩展。
7. JWT logout 使用 Redis 黑名单保存未过期 token 的剩余 TTL，解决无状态 token 退出后仍可能可用的问题。
8. Flyway 管理数据库结构版本，避免不同环境手动执行 SQL 造成表结构漂移。
9. bad case feedback 和 RAG evaluation dataset 用于记录问题样例、期望答案和覆盖情况，形成持续优化闭环。
10. retrieval evaluation 会批量执行标准问题检索，用人工标注的相关文档作为 gold label 计算 Hit@3、MRR 和首个相关片段排名；命中关键词只作为诊断信息展示，不作为相关性裁判。
11. retrieval evaluation 会显式返回检索策略、候选池大小和相关性口径，便于后续把外部 embedding / vector store 与当前关键词全文检索和本地稀疏向量 baseline 做同集对比。
12. embedding 输入文本由 `EmbeddingTextBuilder` 统一从标题、来源类型、标签和 chunk 内容构造，避免索引、重排和后续向量化使用不同字段。
13. 第一版优先支持 TXT / Markdown 导入，保证核心链路稳定后再扩展 PDF、Word、OCR 等解析能力。
...
基于 Spring Boot + Vue 3 实现知识文档管理、TXT/Markdown 导入、自动分块、MySQL FULLTEXT + 多关键词检索、本地稀疏向量持久化重排、Prompt 构造、DeepSeek 调用、引用来源追踪、token 成本观测、问答日志、bad case 反馈和基于 gold label 的 Hit@3/MRR 标准问题检索评估。
```
...
1. 检索增强：外部 embedding、向量库、RRF/混合检索、rerank。
