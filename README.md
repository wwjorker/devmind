...
当前版本不宣称已经实现外部 embedding、向量数据库、专业 rerank、PDF/OCR 或生产级部署。第一版重点是先打通 Java 后端工程链路、RAG 问答闭环和质量评估流程；当前已加入本地稀疏向量持久化、RRF 融合排序与混合检索雏形，为后续接真实向量检索做准备。
...
- chunk 本地稀疏向量持久化、余弦相似度与 RRF 融合排序
...
基于 Spring Boot + Vue 3 实现知识文档管理、TXT/Markdown 导入、自动分块、MySQL FULLTEXT + 多关键词检索、本地稀疏向量持久化与 RRF 融合排序、Prompt 构造、DeepSeek 调用、引用来源追踪、token 成本观测、问答日志、bad case 反馈和基于 gold label 的 Hit@3/MRR 标准问题检索评估。
```
...
1. 检索增强：外部 embedding、向量库、专业 rerank、更大的标准问题集对比。
