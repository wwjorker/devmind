...
核心结论：没有单一最优策略。dense embedding 的价值在精确排序，rerank 的价值在最坏情况召回（Hit@3 满分），但 rerank 会把部分本来排第 1 的结果挤到第 2，拉低平均 MRR。**pgvector 迁移无召回回归**：同一批向量下 HNSW serving 与暴力余弦的 Hit@3 完全一致，MRR 的微小差异来自排序精度——JVM 路径把相似度圆整为整数分会产生并列，pg 按原始距离排序。注意 rerank 是在混合检索 top-5 候选池上做"5 选 3"重排，其 Hit@3 满分受候选池限制；正例样本 35 条，Hit@3 相邻两档相差约 0.029，以上数字为方向性而非统计显著。
