...
DevMind 是一个面向国内 Java 后端直接求职的 AI 知识库项目。
...
简历包装时不要写成“实习生练手项目”，也不要夸成“生产级 AI Agent 平台”。更稳的定位是：

```text
面向 Java 后端求职的完整项目：传统后端工程能力 + RAG 应用链路 + AI 调用可观测 + 评估反馈闭环。
```
...
建议触发 Claude Code 审查的节点：

- JDK 17、本地启动、GitHub Actions、README 和演示截图都稳定后。
- 检索质量升级完成后，例如 Hit@3/MRR、向量检索、rerank 任一阶段完成后。
- 正式投简历前，用它做一次代码、文档、简历描述一致性检查。

不建议每做一个小按钮或小文案都立刻找 Claude Code 审查，否则会把时间耗在反复评审上。
...
- retrieval evaluation 的通过率、Hit@3 和 MRR。
- 前端可视化演示。
...
- rerank。
- PDF/OCR 导入。
- 更大的标准评测集、bad case 分类统计和模型回答质量评测。
- 更细粒度的权限和限流。
...
- 更强评估：当前已完成标准问题检索评估、Hit@3 和 MRR，后续补更大标准评测集、bad case 分类和答案质量评估。
